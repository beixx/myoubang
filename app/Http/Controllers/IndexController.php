...
class IndexController extends Controller
{
    ...
    public function index($name = 'beijing' ,$id = 'sheying' )
    {
        ...
        if($id!='sheying' && $id!="hunli"){
            ...
            $title = $tenants['positionCity'].$tenants['name'].' | 有榜「第'.$tenants['order_city'].'名」';
            $desc = $tenants['positionCity'].$tenants['name'].'在《有榜婚嫁行业榜单》综合排名第'.$tenants['order_city'].'名，该商户在品牌榜单中排名第'.$tenants['brand_search_order'].'名，好评榜单中排名第'.$tenants['praise_order'].'名，希望能够帮助您了解到'.$tenants['name'].'怎么样的问题。';
            $keyword = $tenants['positionCity'].$tenants['name'].', '.$tenants['name'].', '.$tenants['name'].'怎么样, '.$tenants['name'].'行业第'.$tenants['order_city'].'名';
            $this->data = [
                'tenants' => $tenants,
...
//POST请求函数
    ...
    public function wenda($id){
        ...
        $this->getSpread($tenants);
        $this->data['askcity'] = [];
        $this->data['other'] = YfcAsk::where("id","!=",$id)->where("tid",$tenantsId)->orderby("id","desc")->limit(14)->get();
